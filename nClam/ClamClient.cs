namespace nClam
{
	...
	public class ClamClient : IClamClient
	{
		...
		private async Task<string> ExecuteClamCommandAsync(string command, CancellationToken cancellationToken, Func<Stream, CancellationToken, Task>? additionalCommand = null)
		{
			...
			try
			{
				using var stream = await CreateConnectionAsync(clam).ConfigureAwait(false);
		...
		private string ExecuteClamCommand(string command, Action<Stream>? additionalCommand = null)
		{
#if DEBUG
			var stopWatch = System.Diagnostics.Stopwatch.StartNew();
#endif
			...
			try
			{
				using var stream = CreateConnection(clam);
				var commandText = $"z{command}\0";
				var commandBytes = Encoding.UTF8.GetBytes(commandText);
		...
		private void SendStreamFileChunks(Stream sourceData, Stream clamStream)
		{
			var streamSize = 0;
			int readByteCount;
			var bytes = new byte[MaxChunkSize];

			while ((readByteCount = sourceData.Read(bytes, 0, MaxChunkSize)) > 0)
			{
				streamSize += readByteCount;
				...
				var readBytes = BitConverter.GetBytes(System.Net.IPAddress.HostToNetworkOrder(readByteCount));  //convert readByteCount to NetworkOrder!
				clamStream.Write(readBytes, 0, readBytes.Length);
				clamStream.Write(bytes, 0, readByteCount);
			}
		...
		protected async virtual Task<Stream> CreateConnectionAsync(TcpClient clam)
		{
		...
		protected virtual Stream CreateConnection(TcpClient clam)
		{
			if (ServerIP == null)
				clam.Connect(Server, Port);
			else
				clam.Connect(ServerIP, Port);
